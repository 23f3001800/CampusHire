      ...
      <div class="collapse navbar-collapse" id="navbarMain">
        ...
        <ul class="navbar-nav align-items-lg-center gap-1">
          ...
          <template v-if="!store.isAuthenticated">
            <li class="nav-item">
              <router-link to="/login"  class="btn btn-outline-light btn-sm px-3">Sign In</router-link>
            </li>
            <li class="nav-item ms-1">
              <router-link to="/signup" class="btn btn-primary btn-sm px-3">Sign Up</router-link>
            </li>
          </template>
          ...
          <li v-else class="nav-item dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow py-2">
              ...
              <!-- Dropdown-only links (not already in primary nav) -->
              <li v-for="link in navConfig.dropdown" :key="link.name">
                <router-link :to="resolveLink(link)" class="dropdown-item py-2">
                  <i :class="`bi ${link.icon} me-2 text-primary`"></i>{{ link.label }}
                </router-link>
              </li>

              <li v-if="navConfig.dropdown.length"><hr class="dropdown-divider my-1"></li>
              <li>
...
//
// primary  : shown in the top bar for all screen sizes
// dropdown : shown ONLY in the account dropdown (prevents duplicate links)
//
// Rules to avoid duplicates:
//   - If a link appears in `primary`, it MUST NOT appear in `dropdown`
//   - dynamic: true  → inject :id param from userStore.id at runtime
// ─────────────────────────────────────────────────────────────────────────────
const NAV = {
  student: {
    primary: [
    ...
    dropdown: [
      { label: 'Placement History', icon: 'bi-trophy',          name: 'StudentPlacements' },
    ],
  },
...
export default {
  ...
  computed: {
    navConfig() {
      return NAV[this.store.role] || { primary: [], dropdown: [] }
    },
    ...
    roleBadge() {
      return {
        ...
        company: 'bg-success',
        admin:   'bg-danger',
      }[this.store.role] || 'bg-secondary'
    },
  },

  methods: {
    // Always returns a Vue Router location object — never a raw href string.
    // This ensures router-link active-class detection works correctly.
    resolveLink(link) {
      if (link.dynamic) {
        return { name: link.name, params: { id: this.store.id } }
      }
...
/* Dropdown */
.dropdown-menu      { border: none; border-radius: 10px; min-width: 220px; box-shadow: 0 8px 24px rgba(0,0,0,.15) !important; }
.dropdown-item      { font-size: .9rem; border-radius: 6px; margin: 1px 4px; width: calc(100% - 8px); }
