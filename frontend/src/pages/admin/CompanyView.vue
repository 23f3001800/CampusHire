            ...
            <div class="card border-0 shadow-sm">
              ...
              <div class="card-body p-0">
                ...
                <div v-else-if="!drives.length"
                  ...
                  <i class="bi bi-folder-x d-block mb-1
                             fs-4"></i>
                  ...
                  <table class="table table-hover mb-0
                    ...
                    <tbody>
                      <tr v-for="d in drives" :key="d.id">
                        ...
                        <td>
                          <span :class="{
                            'text-danger fw-semibold': isUrgent(
          ...
          <div class="col-lg-4 d-flex flex-column gap-3">
            ...
            <div class="card border-0 shadow-sm">
              ...
              <div class="card-body d-grid gap-2">
                ...
                <!-- Approval actions -->
                <template v-if="company.approval_status
                                 === 'Pending'">
                ...
                <!-- Re-review already-decided companies -->
                <template v-else>
                  <button
...
// ── Computed drive stats ──────────────────────────────────────────────────
const openCount = computed(
  () => drives.value.filter(d => d.status === 'Open').length
)
const closedCount = computed(
  () => drives.value.filter(d => d.status === 'Closed').length
)
const completedCount = computed(
  () => drives.value.filter(d => d.status === 'Completed').length
)
...
const totalApplicants = computed(
  () => drives.value.reduce(
    (sum, d) => sum + (d.total_applications ?? 0), 0
  )
)

// ── Data loading ──────────────────────────────────────────────────────────
// GET /company/:companyId — returns companyfields directly (no .data)
async function loadCompany() {
  loading.value = true
  error.value   = ''
  try {
    company.value = await api.get(`/company/${companyId}`)
    if (!company.value)
      throw new Error('Company not found.')
    // Load drives in parallel, non-blocking
    loadDrives()
  } catch (e) {
...
// GET /company/:companyId/drives — returns drivefields array directly
async function loadDrives(force = false) {
  if (!force && drives.value.length) return
  loadingDrives.value = true
  try {
    const res = await api.get(`/company/${companyId}/drives`)
    // Normalise: backend returns array directly
    drives.value = Array.isArray(res) ? res : (res?.drives ?? [])
  } catch {
    drives.value = []
  } finally {
...
// ── Approval — PUT /admin/companies/:id/approval ──────────────────────────
async function setApproval(status) {
  if (status === 'Rejected' &&
      !confirm('Reject this company? They will be notified.'))
    return
  approving.value = true
  try {
    // adminStore action patches the companies list + decrements stat
    await store.approveCompany
      ? (status === 'Approved'
          ? await store.approveCompany(companyId)
          : await store.rejectCompany(companyId))
      : null
    // Also patch local copy
    company.value = {
      ...company.value, approval_status: status
    }
    showToast(
      'success',
...
// ── Block / Unblock — PUT /admin/users/:userId/active ─────────────────────
async function toggleActive() {
  const newActive = !company.value.is_active
  if (!newActive &&
      !confirm('Block this company? They cannot log in.'))
    return
  toggling.value = true
  try {
    // PUT /admin/users/:userId/active — AdminUserActiveResource
    await api.put(
      `/admin/users/${company.value.user_id}/active`,
      { active: newActive }
    )
    company.value = { ...company.value, is_active: newActive }
    showToast(
      'success',
      `Company ${newActive ? 'unblocked' : 'blocked'}.`
    )
  } catch (e) {
    showToast('danger', e?.message ?? 'Action failed.')
  } finally {
